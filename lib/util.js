...
class Paper {
  ...
  static A3Plus = { width: 329, height: 483 }; // Approximate size, not standard
...
class Palette {
  constructor(colorHexArr) {
    this.palette = colorHexArr.map(color => RGBA.hexToRgb(color));
  }
  ...
  getClosest(inColor) {
    ...
    this.palette.forEach(col => {
      const dist = inColor.distance(col);
...
module.exports = { Vector, Paper, RGB, RGBA, Palette }